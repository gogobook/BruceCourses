...
Ibles.signupBannerTemplate += "		<a href=\"\/account\/register?sourcea=banner&bannerType=<< bannerType >>&utm_medium=cta&tum_source=popup&utm_campaign=signup\" class=\"signup-prompt-submit submit-form btn btn-orange\"><< buttonText >><\/a>";
...
Ibles.feedPromptTemplate += "        <button type=\"button\" data-dismiss=\"modal\" class=\"close alert-close\"><span>x<\/span><\/button>";
...
Ibles.giftProPromptTemplate="";
Ibles.giftProPromptTemplate += "    <div class=\"call-to-action << display >> gift-pro-prompt\" data-type=\"type-<< bannerType >>\" style=\"display:block\">";
Ibles.giftProPromptTemplate += "        <button type=\"button\" class=\"close alert-close\"><span>x</span><\/button>";
Ibles.giftProPromptTemplate += "        <div class=\"call-to-action-header\"><\/div>";
Ibles.giftProPromptTemplate += "        <div class=\"call-to-action-form\">";
Ibles.giftProPromptTemplate += "            <p><< message >><\/p>";
Ibles.giftProPromptTemplate += "            <a href=\"\/account\/give?proItem=ONE_YEAR_ONCE&sourcea=gifts\" class=\"submit-form btn btn-orange gift-pro-link\"><< buttonText >><\/a>";
Ibles.giftProPromptTemplate += "        <\/div>";
Ibles.giftProPromptTemplate += "    <\/div>";
...
Ibles.mobileTemplate += "					<a href=\"\/account\/register?sourcea=banner&bannerType=<< bannerType >>&utm_medium=cta&tum_source=popup&utm_campaign=signup\" class=\"signup-prompt-submit submit-form btn btn-orange\"><< buttonText >><\/a>";
    ...
    submitForm: function(e){
        ...
        if (email) {
            $.ajax({
                ...
                method: "GET",
                success: function() {
                    window.location = "/account/register?sourcea=banner&sourceaUrl="+sourceaUrl+"&bannerType="+bannerType+"&email="+email+"&utm_medium=cta&tum_source=popup&utm_campaign=signup";
;
                }
            });
        } else {
            window.location = "/account/register?sourcea=banner&sourceaUrl="+sourceaUrl+"&bannerType="+ bannerType+"&utm_medium=cta&tum_source=popup&utm_campaign=signup";
        }
    },
    setClosedCookie: function(){
...
Ibles.views.GiftProPrompt = Ibles.views.CallToActionView.extend({
    template: _.template(Ibles.giftProPromptTemplate),
    initialize: function(options){
        this.render();
    }
});
...
Ibles.showGiftProPrompt = function(model, delay) {
    var giftPro_model = new Ibles.models.CallToAction(model);

    var view = new Ibles.views.GiftProPrompt({model:giftPro_model});
    $("body").append(view.$el);

    setTimeout(function(){
        view.show();
        window.dataLayer = window.dataLayer || [];
        dataLayer.push({'sourcea' : 'gifts'});
    }, delay);
};
